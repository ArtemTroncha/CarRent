...
<form @submit.prevent="GetBrands">
  ...
  <div class="row">
    ...
    <div class="col mb-5">
      ...
      <input  type="file" class="form-control" id="customFile" ref="CarImg"/>
      
  </div>
  
  <div class="row">
    <div class="col mb-5">
      <button @click="ChangeCar" type="button" class="btn  btn-dark">Change Car</button>
    </div>
  </div>
    </div>
    </div>
     ...
     methods:{
      ...
      async created(){
         ...
         .then((res)=>{
          ...
          await PostService.GetById(this.$route.params.id)
          .then((res)=>{
            const post= res[0]
          ...
          this.FuelCons=post.fuel_consumption,
          this.seats=post.seat_count,
          this.FuelType=post.fuel_type,
          this.description=post.discription
